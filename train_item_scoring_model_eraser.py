...
from captum.attr import Saliency, IntegratedGradients, InputXGradient, configure_interpretable_embedding_layer, remove_interpretable_embedding_layer
from interpretability.custom_gradient import compute_gradients
...
def parse_args():
    ...
    parser.add_argument("--penalty_loss", "-plt", dest="plt", default=False,
                        action='store_true', help="")
    parser.add_argument("--emb", dest="emb_path", type=str, metavar='<str>', default=None,
    ...
    parser.add_argument("--train_grad_with", action="store_true",
                        default=False, help="attentionの学習と一緒にgradientも学習する")
    parser.add_argument("--grad_loss_ratio", default=1.0,
                        type=float, help="ratio of gradient_norm loss")
    parser.add_argument("--sup_grad", choices=["saliency_map", "input_x_gradient",
                                               "integrated_gradients", None], type=str, default=None)
    parser.add_argument("--stop_train_scoring", default=False,
                        action='store_true', help="得点予測での学習をストップする")
    parser.add_argument("--use_loss_for_eval",
                        default=False, action="store_true")
    parser.add_argument("--after_emb",
                        default=False, action="store_true", help="bertの得と湯マップを，bertの後で作成")
...
def main():
    ...
    logger.info(f"Optimizer: {optimizer}, {info.opt}")
    ...
    dev_best_score_loss = float('inf')
    test_best_score_loss = float('inf')
    dev_best_attn_loss = float('inf')
    test_best_attn_loss = float('inf')
    dev_best_grad_loss = float('inf')
    test_best_grad_loss = float('inf')
    ...
    if args.sup_grad is not None:
        if args.sup_grad == "saliency_map":
            method = Saliency(model.predict)
        elif args.sup_grad == "input_x_gradient":
            method = InputXGradient(model.predict)
        elif args.sup_grad == "integrated_gradients":
            method = IntegratedGradients(model.predict)
        else:
            raise ValueError(args.sup_grad)
    ...
    if args.reg:
        eval = evaluator_reg.EvaluatorForItem(
            info, dev_x, test_x, dev_y, test_y, dev_mask, test_mask, item_num)
    else:
        eval = evaluator_cls.EvaluatorForItem(
            info, dev_x, test_x, dev_y, test_y, dev_mask, test_mask, dev_attention, test_attention, item_num, criterion, method)
    ...
    for ii in range(args.epochs):
        ...
        for batch_x, batch_y, batch_m, batch_a, batch_f in dataloader:
            ...
            if not args.stop_train_scoring:
                if args.plt:
            ...
            if info.satt:
                if args.sup_grad is not None:
                    with torch.backends.cudnn.flags(enabled=False):
                        ...
                        assert grad_norm.size() == batch_x.size()
                        # grad_loss = attn_loss(
                        #     grad_norm, batch_a_ps) * args.grad_loss_ratio
                        grad_loss += attn_loss(grad_norm[batch_f],
                                               batch_a_ps[batch_f])
                        batch_gradient_loss += grad_loss * batch_x.shape[0]
                        loss += grad_loss * args.grad_loss_ratio

                if not (args.sup_grad is not None and not args.train_grad_with):
        ...
        if not args.stop_train_scoring:
            logger.info(
                f"Scoring loss: {batch_scoring_loss/len(dataloader):.3f}")
        if info.satt:
            logger.info(
                f"Attention loss: {batch_attention_loss/len(dataloader):.3f}")
        if args.sup_grad is not None:
            logger.info(
                f"gradient loss: {batch_gradient_loss/len(dataloader):.3f}")

        del batch_x, batch_y, batch_m, batch_a, batch_f

        model.eval()
        with torch.no_grad():
            (dev_qwks, test_qwks, dev_mses, test_mses, dev_score_loss, test_score_loss, dev_attn_loss, test_attn_loss, dev_grad_loss, test_grad_loss), train_hidden_states, train_golds, (dev_attentions, dev_hidden_states), (test_attentions,
                                                                                                                                                                                                                               test_hidden_states), d_pred, t_pred, dev_target, test_target, (dev_outputs, test_outputs), (_) = eval.evaluate(info, model, eval_dataloader, accuracy_mode=args.accuracy_mode, is_test=False, use_loss_for_eval=args.use_loss_for_eval)
            # logger_eval.info(f"[Dev]: {dev_qwk:.3f} [Test]: {test_qwk:.3f}")
            # (dev_qwks, dev_mses), d_pred, dev_target = eval._evaluate(info, model, eval_dataloader, accuracy_mode=args.accuracy_mode, mode="dev")

            if args.use_loss_for_eval:
                best_loss = 0
                dev_loss = 0
                dev_loss += dev_score_loss
                best_loss += dev_best_score_loss
                dev_loss += dev_attn_loss
                best_loss += dev_best_attn_loss
                # 勾配も学習している時は勾配のlossも使う
                if args.sup_grad is None:
                    dev_loss += dev_grad_loss
                    best_loss += dev_best_grad_loss

                is_best = dev_loss < best_loss
            else:
                is_best = best < dev_qwks

            if is_best or args.stop_train_scoring:
                # args.stop_train_scoringはdebugのために絶対モデルを保存する
                # (test_qwks, test_mses), t_pred, test_target = eval._evaluate(info, model, eval_dataloader, accuracy_mode=args.accuracy_mode, mode="test")
                ...
                dev_best_score_loss = dev_score_loss
                dev_best_attn_loss = dev_attn_loss
                dev_best_grad_loss = dev_grad_loss
                test_best_score_loss = test_score_loss
                test_best_attn_loss = test_attn_loss
                test_best_grad_loss = test_grad_loss
                ...
                best_test_attentions = None
                best_test_hidden_states = None
                best_dev_attentions = None
                best_dev_hidden_states = None
                ...
                #     handling_data.save_hidden_states_item(
                #         info, None, pred=d_pred, attn=best_dev_attentions, norm=best_dev_norms, grad=best_dev_grads_norm, target=dev_target[item_num+1], prefix="dev")
                #     handling_data.save_hidden_states_item(
                #         info, None, pred=t_pred, attn=best_test_attentions, norm=best_test_norms, grad=best_test_grads_norm, target=test_target[item_num + 1], prefix="test")
    ...
    if args.save_fin:
        ...
        handling_data.save_hidden_states_item(
            info, None, pred=d_pred, target=dev_target[item_num + 1], prefix="dev_final")
    ...
    if args.use_loss_for_eval:
        eval.print_loss_info(dev_best_score_loss, dev_best_attn_loss, dev_best_grad_loss, item_num,
                             type="Dev", log=logger)
        eval.print_loss_info(test_best_score_loss, test_best_attn_loss, test_best_grad_loss,
                             item_num, type="Test", log=logger)

    else:
        eval.print_info(best, best_mse, item_num, type="Dev", log=logger)
        eval.print_info(test_best, test_mses, item_num,
    ...
    if args.reg:
        ...
        handling_data.save_result_item_scoring_cls(
            info, best_test_pred, test_mask.sum(1), info.test_dir, item_num, probs=True, attentions=best_test_attentions, prefix="test", norms=None)
        handling_data.save_result_item_scoring_cls(
            info, best_dev_pred, dev_mask.sum(1), info.dev_dir, item_num, probs=True, attentions=best_dev_attentions, prefix="dev", norms=None)
    ...
    if args.make_feature_map:
        # best modelをロードする
        from analysis.util import load_model_eraser
...
def regularization(target_embeddings):
    return target_embeddings / torch.sum(target_embeddings, dim=-1).view(-1, 1)
