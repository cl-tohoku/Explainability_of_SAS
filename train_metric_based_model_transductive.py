...
def main():
    ...
    from model import BertForPartScoringWithLSTMTransductive, BiRnnModel
    ...
    if args.BERT:
        from sas.input_data import input_data_ps_BERT_transductive as input_data
    else:
        from sas.input_data import input_data_ps_transductive as input_data
    ...
    if args.BERT:
        ...
        model = BertForPartScoringWithLSTMTransductive(config, t_info=info)
    ...
    if args.imp:
        ...
        logger.info(f"Loss: Tripled Loss")
    ...
    import evaluator_transductive

    logger.debug(f"train shape:{train_x.shape}")
    logger.debug(f"dev shape: {dev_x.shape}")
    logger.debug(f"test shape:{test_x.shape}")

    eval = evaluator_transductive.Evaluator(
        info, dev_x.to(device), test_x.to(device),dev_y, test_y, dev_mask.to(device), test_mask.to(device),dev_attention.to(device),test_attention.to(device))
    ...
    for ii in range(args.epochs):
            ...
            for i, label in enumerate(info.ps_labels):
               ...
               # tripletが作れルト気時
                ...
                if i < len(info.main_factors) and info.satt:
                    ...
                    loss_satt += attn_loss(attn_output[i],batch_a_ps[:, :p_len])
        ...
        logger.info(
            f"Scoring loss: {t_loss.item()/len(dataloader):.3f} Attention loss: {sum(b_loss)/len(b_loss) / len(info.ps_labels):.3f}")
    ...
    handling_data.save_result_part_scoring_metric_transductive(
