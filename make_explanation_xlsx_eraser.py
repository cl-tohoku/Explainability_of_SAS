...
def parse_args():
    ...
    parser.add_argument("--not_print_discrete", '-npd', action="store_true",
                        default=False, help="don't print discrete justifiation")
    parser.add_argument("--size", default=None,
                        type=int, help="size of answer")
    parser.add_argument("--flip_mode", default=False, action="store_true")
...
def print_discrete_justification_data(sentence_list, justification_list, workbook, sheet_name: str, id: str, pred_score: int, gold_score: int, remove_ratio: int = None, threshold=0.5):
    ...
    row_data = [[id], answer, [pred_score], [gold_score], [remove_ratio]]
    workbook.write_row_data_to_xlsx(sheet_name, row_data)
    return


def print_continuous_justification_data(sentence_list, justification_list, workbook, sheet_name, id, pred_score, gold_score, remove_ratio: int = None):
    # assert len(sentence_list) == len(
    #     justification_list), f"{len(sentence_list)}, {len(justification_list)}"
    if len(sentence_list) != len(justification_list):
        print(sentence_list)
    ...
    row_data = [[id], soft_answer,
                [pred_score], [gold_score], [remove_ratio]]
    workbook.write_row_data_to_xlsx(sheet_name, row_data)
    return
...
def main():
    ...
    column_name_list = [["id"], ["答案"], ["pred"], ["gold"], ["削除率"]]
    ...
    sanity_file = f'''{info.out_dir}_faithfulness_eraser_flip_mode.pickle''' if args.flip_mode else f'''{info.out_dir}_faithfulness_eraser_zero_score.pickle'''
    sanity_df = pickle.load(
        open(sanity_file, "rb")) if os.path.isfile(sanity_file) else None
    ...
    for mode in ["train", 'test', 'dev']:
        for idx, (sentence_list, gold_attention_list, gold_score, pred_score, id) in enumerate(get_moview_review_list_for_xlsx(info, mode, args.size)):
            # goldデータを出力
            ...
            if mode != 'train':
                for method, maps in get_iteration_explanation_pickle_data(info, mode, idx):
                    # remove_ratio = sanity_df[sanity_df["name"] ==
                    #                          method].iloc[idx]["flip_most"] if sanity_df is not None and mode == "test" else None
                    remove_ratio = sanity_df[method].iloc[idx] if sanity_df is not None and mode == "test" and pred_score != 0 else None
                    if args.methods is None or method in args.methods:
                        ...
                        if not args.not_print_discrete:
                            print_discrete_justification_data(sentence_list, maps, workbook, mode,
                                                              f"{id}-{method}-d", pred_score, gold_score, remove_ratio, justi_result_data[method]["test"]["threshold"],)
                        print_continuous_justification_data(
                            sentence_list, maps, workbook, mode, f"{id}-{method}-c", pred_score, gold_score, remove_ratio)
