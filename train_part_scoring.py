...
def main():
    ...
    import evaluator_cls,evaluator_reg
    ...
    for ii in range(args.epochs):
        ...
        for batch_x, batch_y, batch_m, batch_a in dataloader:
            ...
            for i, label in enumerate(info.ps_labels):
                if args.reg:
                    batch_y_ps = batch_y[i +1].type(torch.FloatTensor).abs().to(device)
                    output[i] = torch.squeeze(output[i])
                else:
                ...
                if i < len(info.main_factors) and info.satt:
                    ...
                    loss_satt += attn_loss(attn_output[i],
                                           batch_a_ps[:, :p_len])
                if args.plt:
                    loss += criterion(output[i], batch_y_ps, device)
                else:
                    loss += criterion(
                        output[i], batch_y_ps)
