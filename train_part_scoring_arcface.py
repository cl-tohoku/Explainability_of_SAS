...
def parse_args():
    ...
    parser.add_argument("--metric", dest="metric", type=str, metavar='<str>',
                        help="The path to the word embeddings file (Word2Vec format)")
...
def main():
    ...
    from model import BertForScoring, BertForPartScoring, BertForPartScoringWithLSTM, BiRnnModel, BiRnnModelWithMetricFc
    ...
    if args.BERT:
        ...
        model = BiRnnModelWithMetricFc(info, args.metric)
    ...
    for ii in range(args.epochs):
        ...
        for batch_x, batch_y, batch_m, batch_a in dataloader:
        ...
        with torch.no_grad():
            (dev_qwks, test_qwks, dev_mses, test_mses), (dev_attentions, dev_hidden_states), (
                test_attentions, test_hidden_states), d_pred, t_pred, dev_target, test_target = eval.evaluate(info, model)
            ...
            if best["Hol"] < dev_qwks["Hol"]:
                ...
                handling_data.save_hidden_states(
                    info, dev_hidden_states[0], d_pred, dev_target, prefix="dev")
                handling_data.save_hidden_states(
                    info, test_hidden_states[0], t_pred, test_target, prefix="test")
    ...
    handling_data.save_result_part_scoring_cls(
        info, best_test_pred, test_mask.sum(1), probs=True, attentions=best_test_attentions, hidden_states=best_test_hidden_states, prefix="test")
    model.load_state_dict(torch.load(info.out_dir +
                                     "_best_model_weights.pt"))
    model.eval()
    handling_data.save_train_result_part_scoring_metric_fc(
